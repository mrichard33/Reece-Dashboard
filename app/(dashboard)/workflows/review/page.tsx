                          ...
                          return (
                            <li key={w.ghlWorkflowId} className="flex flex-wrap items-baseline gap-x-2">
                              ...
                              <span className="text-xs text-slate-500">
                                ...
                                {row?.sending?.verdict === "turned_off" ? " · messages turned off in GHL" : row?.sending?.verdict === "no_sends_seen" ? " · no sends seen" : row?.sending?.sends30d != null ? ` · ${num(row.sending.sends30d)} sent / 30 d` : ""}
                      ...
                      .map((w) => {
                        ...
                        return (
                          <tr key={w.ghlWorkflowId}>
                            ...
                            <td className="px-3 py-1.5 text-xs tabular-nums">{row?.sending?.verdict === "turned_off" ? "turned off" : row?.sending?.verdict === "no_sends_seen" ? "no sends seen" : row?.sending?.sends30d != null ? num(row.sending.sends30d) : "—"}</td>
                          </tr>
                        );
                      })}
                  </tbody>
