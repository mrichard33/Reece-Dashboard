...
describe("issue-rate fallback from current report snapshots", () => {
  ...
  test("Orlando is (ORL+LAKE issued) ÷ (ORL+LAKE leads) — a ratio of sums", () => {
    const r = issueRateFromFacts(LIVE, marketSources("ORL_MKT"));
    expect(r.issued).toBe(2948 + 439);
    expect(r.leads).toBe(16527 + 28 + 2899 + 28);
    expect(r.rate).toBeCloseTo(3387 / 19482, 4);
    // NOT the average of the two markets' own rates
    const avgOfRates = (2948 / 16555 + 439 / 2927) / 2;
    expect(r.rate).not.toBeCloseTo(avgOfRates, 4);
  });
